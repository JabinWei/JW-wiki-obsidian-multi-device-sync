...
![[90-Storage/98-Asset-资源/CoverImage/PY008-简单数据类型/Python简单数据类型-coverImage-v1.png|banner]]
...
- [[PY005-变量]]、[[PY007-运算符]] — 掌握变量定义、赋值语法和基本运算符的使用
...
![[90-Storage/98-Asset-资源/InfoGraphic/PY008-简单数据类型/Python数据百宝箱-信息图-v1.png|Python 简单数据类型全景图]]
...
- **原生字符串 `r""` **：忽略转义字符，处理正则或路径（如 `r"C:\new"`）时必备。在文件操作中，使用原生字符串定义文件路径可以避免 `\` 被误解析为转义字符，详见 [[PY018-文件]]。
...
你可能遇到过 `0.1 + 0.2 != 0.3` 的情况。这并非 Python 的 Bug，而是计算机**二进制存储机制**的通病。详见 [[PL-006-浮点数精度误差的二进制根源]]。
