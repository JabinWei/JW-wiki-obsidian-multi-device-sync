...
![[90-Storage/98-Asset-资源/CoverImage/PY015-高阶函数/Python高阶函数-coverImage-v1.png|banner]]
...
#### 闭包核心原理：`__closure__` 与 `nonlocal`

**它是如何"记忆"的？（`__closure__`）**
...
> 例如：`[x** for x in numbers]` 通常比 `list(map(lambda x: x**2, numbers))` 更受欢迎。详见 [[PL-PY-006-Python推导式全家桶]]。
...
![[90-Storage/98-Asset-资源/ArticleIllustrator/PY015-高阶函数/01-illustration-yield-lifecycle.png|yield 生成器生命周期]]
...
![[90-Storage/98-Asset-资源/ArticleIllustrator/PY015-高阶函数/02-illustration-return-vs-yield.png|return vs yield 对比]]
...
```python
funcs = [lambda: x**for x in range(1, 6)]
for f in funcs:
    print(f())
```
...
>
> ```python
> funcs = [lambda x=x: x**for x in range(1, 6)]
> for f in funcs:
>     print(f())
> # 输出:
> # 1
...
> result2 = sum(x**for x in nums if x % 2 == 0)
...
- [[PL-PY-006-Python推导式全家桶]] — 推导式与 map/filter 对比、生成器表达式
- [[PL-PY-012-可迭代对象与迭代器协议]] — 可迭代对象、迭代器、生成器三者的协议辨析