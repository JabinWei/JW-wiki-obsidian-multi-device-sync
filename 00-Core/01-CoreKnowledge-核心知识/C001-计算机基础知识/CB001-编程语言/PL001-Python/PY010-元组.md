...
![[90-Storage/98-Asset-资源/CoverImage/PY010-元组/Python元组-coverImage-v1.png|banner]]
...
![[90-Storage/98-Asset-资源/InfoGraphic/PY010-元组/Python元组全景-信息图-v1.png|元组 vs 列表全景对比]]
...
> 详见 [[PL-PY-005-元组不可变性的真正边界]] — 不可变的是引用，不是引用对象本身
...
![[90-Storage/98-Asset-资源/InfoGraphic/PY010-元组/Python元组全景-信息图-v2.png|元组 vs 列表全景对比]]
...
#### 作为字典的键
因为元组是**可哈希的**，它可以作为字典的键。详见 [[PL-PY-004-哈希表的原理与可哈希协议]]。
...
>
> 因为字典的键必须是**可哈希的**（即不可变的），元组本身是可哈希的（A、C、D 都正确），但 B 中的元组 `([1, 2], [3, 4])` 包含两个列表，列表是不可哈希的，所以整个元组也不可哈希，无法作为字典键。详见 [[PL-PY-004-哈希表的原理与可哈希协议]]。
>
> **验证哈希性：**
> ```python
...
**事实**：元组的不可变是指**元素引用不可变**，即不能将某个索引重新绑定到新对象。但如果元组的元素本身是可变对象（如列表、字典），这些对象的内部状态仍然可以修改。详见 [[PL-PY-005-元组不可变性的真正边界]]。
...
- 下一节：[[PY011-字典]] — 字典的键要求可哈希，元组正是满足这一条件的复合类型。详见 [[PL-PY-004-哈希表的原理与可哈希协议]]
...
- **卡片知识**：
	- [[PL-PY-003-下划线占位符的使用]] — 解包中用 `_` 和 `*_` 丢弃不需要的值
	- [[PL-PY-005-元组不可变性的真正边界]] — 元组不可变的是引用，不是引用对象本身
