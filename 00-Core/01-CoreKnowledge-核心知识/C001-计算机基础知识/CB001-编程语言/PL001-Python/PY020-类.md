...
![[90-Storage/98-Asset-资源/CoverImage/PY020-类/Python类-coverImage-v1.png|banner]]
...
类中的函数被称为**方法**。创建类时，最常用的是 `__init__()` 方法，这是一个特殊的方法，在创建新实例时会自动运行。`__new__` 与 `__init__` 的分工详见 [[PL-PY-009-__new__与__init__]]。
...
![[90-Storage/98-Asset-资源/ArticleIllustrator/PY020-类/01-illustration-class-lifecycle.png|Python 类的生命周期]]
...
> [!info]- 答案
>  Python 中没有真正的私有属性。`__balance` 通过名称改写（name mangling）变成了 `_BankAccount__balance`，这是一种约定而非强制。应通过 `get_balance()` 等公开方法访问。
---
...
> [!info]- 答案
>  方案B（组合）更合理。人「拥有」一本书是「has-a」关系，应该用组合；人「是」一本书是「is-a」关系才适合用继承。错误使用继承会导致语义混乱和耦合过紧。
---
...
> [!info]- 答案
>  `total` 是类变量，属于类本身，所有实例共享同一个值。`name` 是实例变量，每个实例各自持有。通过 `Counter.total` 访问类变量更清晰；`c1.total` 虽然也能访问，但容易与实例变量混淆。
---
...
- **卡片知识**：[[PL-PY-009-__new__与__init__]] — `__new__` vs `__init__` 分工、单例模式、不可变类型继承
