...
类中的函数被称为**方法**。创建类时，最常用的是 `__init__()` 方法，这是一个特殊的方法，在创建新实例时会自动运行。`__new__` 与 `__init__` 的分工详见 [[PL-PY-009-new-vs-init]]。
...
- **卡片知识**：[[PL-PY-009-new-vs-init]] — `__new__` vs `__init__` 分工、单例模式、不可变类型继承
