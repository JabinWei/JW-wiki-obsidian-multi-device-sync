...
![[90-Storage/98-Asset-资源/CoverImage/PY012-集合/Python集合-coverImage-v1.png|banner]]
...
- **可变性  (Mutable) :** 集合本身是可变的，你可以随时添加或删除元素。但请注意，集合中的**元素本身必须是不可变类型**（即可哈希的），例如数字、字符串、元组。列表或字典这类可变对象不能作为集合的元素。详见 [[PL-PY-004-哈希表的原理与可哈希协议]]。
...
![[90-Storage/98-Asset-资源/ArticleIllustrator/PY012-集合/01-illustration-set-operations.png|集合数学运算韦恩图]]
...
| 运算名称 | 运算符 | 方法 | 描述 | 结果示例 |
| --- | --- | --- | --- | --- |
| **并集** | A &#124; B | `A.union(B)` | 包含所有属于A或属于B的元素（去重） | `{1, 2, 3, 4, 5, 6, 7, 8}` |
...
1. **集合推导式 (Set Comprehension)** 和列表推导式类似，集合推导式提供了一种简洁的方式来创建集合。四种推导式的横向对比详见 [[PL-PY-006-Python推导式全家桶]]。
...
2. **不可变集合 (frozenset)** 标准的 `set` 是可变的，因此它本身是不可哈希的，不能作为另一个集合的元素或字典的键。`frozenset` 就是为了解决这个问题而生的。它一旦创建，内容就无法更改。详见 [[PL-PY-004-哈希表的原理与可哈希协议]]。
...
3. **性能优势：为什么集合查找这么快？** 集合的底层是基于**哈希表 (Hash Table)** 实现的。这意味着检查一个元素是否在集合中（`x in my_set`）的时间复杂度接近 O(1)，无论集合有多大，查找速度都几乎不变。相比之下，在列表中查找元素的时间复杂度是 O(n)，需要逐个遍历，数据量大时速度会慢得多。详见 [[PL-PY-004-哈希表的原理与可哈希协议]]。
...
3. **试图将可变对象放入集合**
   - **错误做法：** `my_set = {[1, 2], [3, 4]}`
   - **事实：** 集合中的元素必须是不可变（可哈希）类型。列表、字典、普通集合本身都不能作为集合的元素。如需嵌套集合，使用 `frozenset`。详见 [[PL-PY-004-哈希表的原理与可哈希协议]]。
...
5. **低估集合查找的性能优势**
   ...
   - **事实：** 集合基于哈希表实现，成员检查时间复杂度为 O(1)；列表需要逐个遍历 O(n)。在大量成员检查的场景下（如循环中反复判断），使用集合可获得数十到数千倍的性能提升。详见 [[PL-PY-004-哈希表的原理与可哈希协议]]。
...
- [[PL-PY-006-Python推导式全家桶]] — 四种推导式横向对比
