...
![[90-Storage/98-Asset-资源/CoverImage/PY011-字典/Python字典-coverImage-v1.png|banner]]
...
**使用字典推导式进行批量/条件删除**（详见 [[PL-PY-006-Python推导式全家桶]]）
...
5. **误区：字典的键可以直接使用列表。**
   - **事实**：字典的键必须是**可哈希（hashable）**的不可变类型。列表是可变类型，不可哈希，不能作为字典的键。可以作为键的类型包括：字符串、数字、元组（元组内的元素也必须全是不可变类型）、`frozenset` 等。详见 [[PL-PY-004-哈希表的原理与可哈希协议]]。
